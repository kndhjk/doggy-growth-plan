...
export default function PhotoUpload({
  ...
  const handleFile = async (e) => {
    ...
    if (!file.type.startsWith('image/')) {
      toast.error(t('photo.error.notImage'));
      return;
    }
    if (file.size > maxBytes) {
      toast.error(t('photo.error.tooBig', { n: Math.round(maxBytes / 1024 / 1024) }));
      return;
    }
    setBusy(true);
    try {
      ...
      toast.error(t('photo.error.uploadFail'));
    } finally {
      setBusy(false);
    }
  };
      ...
      >
        {value ? (
          ...
          <div style={{ textAlign: 'center', color: '#f9a8d4', fontSize: 12, fontWeight: 700 }}>
            ...
            <div>{t('photo.uploadText')}</div>
          </div>
        )}
        {busy && (
          <div style={overlayStyle}>
            <span style={{ fontSize: 12, color: 'white', fontWeight: 700 }}>{t('photo.uploading')}</span>
          </div>
        )}
        {value && !busy && (
          <span
