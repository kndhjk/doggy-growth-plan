...
export default function LoginPage() {
  ...
  const submit = async e => {
    ...
    catch (err) { toast.error(authErrorText(err, t('auth.login.error'))); }
  ...
  return (
    <div style={{ minHeight:'100vh', background:'linear-gradient(160deg,#fdf2f8,#fce7f3)',
      ...
      <h1 style={{ fontSize:24, fontWeight:800, color:'#9d174d', marginBottom:4 }}>{t('auth.login.title')}</h1>
      <p style={{ color:'#f472b6', fontSize:14, marginBottom:32 }}>{t('auth.login.subtitle')}</p>

      <form onSubmit={submit} style={{ width:'100%', maxWidth:360, display:'flex', flexDirection:'column', gap:12 }}>
        <input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder={t('auth.login.email')}
               required style={inputStyle} />
        <input type="password" value={pass} onChange={e=>setPass(e.target.value)} placeholder={t('auth.login.password')}
               required style={inputStyle} />
      ...
      <p style={{ marginTop:24, fontSize:14, color:'#9ca3af' }}>
        ...
        <Link to="/register" style={{ color:'#ec4899', fontWeight:600 }}>{t('auth.login.registerLink')}</Link>
      </p>
    </div>
  );
}
