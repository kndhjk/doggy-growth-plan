...
import { fetchAdoptions } from '../services/api';

// ─── Mock Data ─────────────────────────────────────────────────────────────────
const FALLBACK_ADOPTABLE_PETS = [
  {
...
export default function AdoptPage() {
  ...
  const [pets, setPets] = useState(FALLBACK_ADOPTABLE_PETS);
  ...
  useEffect(() => {
    let cancelled = false;
    fetchAdoptions({ category: typeFilter === 'all' ? 'all' : typeFilter })
      .then((rows) => {
        if (!Array.isArray(rows) || rows.length === 0 || cancelled) return;
        const mapped = rows.map((row, index) => ({
          id: String(row.id),
          name: row.title || `Pet ${index + 1}`,
          breed: row.category === 'cat' ? 'Mixed Cat' : row.category === 'dog' ? 'Mixed Dog' : 'Pet',
          type: row.category === 'cat' ? 'cat' : 'dog',
          age: 'Looking for a home',
          gender: '🐾',
          personality: row.description || 'Friendly and ready for adoption',
          health: 92,
          energy: 78,
          hunger: 25,
          intro: row.description || 'This pet is available for adoption now.',
          tags: [row.category === 'cat' ? 'Cat' : 'Dog', 'Online', row.location || 'Shared listing'],
          bgGradient: row.category === 'cat' ? 'linear-gradient(135deg,#fce7f3,#fbcfe8)' : 'linear-gradient(135deg,#fef3c7,#fde68a)',
          accentColor: row.category === 'cat' ? '#ec4899' : '#f59e0b',
          emoji: row.category === 'cat' ? '🐱' : '🐕',
          features: ['Online listing', row.location || 'Shared across devices', row.sellerName || 'Community owner'],
          adoptionFee: Number(row.price || 0),
        }));
        setPets(mapped);
      })
      .catch(() => {});
