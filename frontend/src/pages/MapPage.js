...
export default function MapPage() {
  ...
  const checkIn = async (place) => {
    ...
    try {
      ...
      try { token = await auth.currentUser?.getIdToken() || ''; } catch {}
      const r = await fetch(
        `${process.env.REACT_APP_API_URL||'http://localhost:5000'}/api/map/checkin`,
        {
          method: 'POST',
  ...
  // Subscribe to Firestore checkins (real users) and merge with local cache.
  useEffect(() => {
    ...
    try {
      ...
      const unsub = onSnapshot(
        ...
        snap => {
          ...
          // Merge by placeId+timestamp window (~1 min), prefer remote
          const local = loadLocalCheckins();
          const merged = [...remote];
          local.forEach(l => {
            const dup = remote.find(r => r.placeId === l.placeId && Math.abs(new Date(r.createdAt?.toDate ? r.createdAt.toDate() : r.createdAt).getTime() - new Date(l.createdAt).getTime()) < 60000);
