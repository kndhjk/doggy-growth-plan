...
export async function fetchAdoptions({ category = 'all' } = {}) {
  const params = new URLSearchParams();
  if (category !== 'all') params.set('category', category);
  return api(`/api/adoptions${params.size ? '?' + params : ''}`);
}

export async function fetchMapCheckins(uid) {
  return api(`/api/map/checkins?uid=${encodeURIComponent(uid)}`);
}

export async function createMapCheckin(payload) {
  return api('/api/map/checkin', { method: 'POST', body: JSON.stringify(payload) });
}

export async function fetchMatchProfile(uid) {
  return api(`/api/community/match-profile?uid=${encodeURIComponent(uid)}`);
}

export async function saveMatchProfile(payload) {
  return api('/api/community/match-profile', { method: 'PUT', body: JSON.stringify(payload) });
}

export async function fetchMatchProfiles(uid) {
  const params = new URLSearchParams();
  if (uid) params.set('uid', uid);
  return api(`/api/community/matches${params.size ? '?' + params : ''}`);
}
