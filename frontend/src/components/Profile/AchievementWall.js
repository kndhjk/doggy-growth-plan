...
const COLLAPSE_KEY = 'gg_achieve_collapsed';
  ...
  return (
    <div style={card}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center',
                    marginBottom: collapsed ? 0 : 12, cursor:'pointer' }}
           onClick={() => setCollapsed(c => !c)}>
        <p style={cardTitle}>{t('achieve.title')}</p>
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          <span style={{ fontSize:12, color:'#9ca3af', fontWeight:700 }}>
            {unlockedCnt} / {total}
          </span>
          <span style={{ color:'#f9a8d4', fontSize:12, fontWeight:800 }}>
            {collapsed ? '▼' : '▲'}
          </span>
        </div>
      </div>

      {!collapsed && <div style={grid}>
        {ACHIEVEMENTS.map(a => {
      ...
      {!collapsed && openKey && (() => {
        const a = ACHIEVEMENTS.find(x => x.key === openKey);
        if (!a) return null;
        return (
          <div style={{
